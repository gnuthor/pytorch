...
class CppWrapperCpuArrayRef(CppWrapperCpu):
    ...
    def _codegen_v2_raw_input_symbols(self, code: IndentedBuffer) -> None:
        ...
        for name, value in inputs:
            self.codegen_input_symbol_assignment(name, value, bound_vars, code=code)
    ...
    def _codegen_v2_raw_outputs(
        ...
        def write_output_to_c_array(idx: int, output: str) -> None:
            ...
            code.writeline(
                f"std::tuple_element_t<{idx}, AOTInductorModelOutputs> {output_arrayref_name};"
            )
            code.writeline(
                f"convert_handle_to_arrayref_tensor({output}, {output_arrayref_name});"
            )
            code.writeline(
                f"torch::aot_inductor::arrayref_tensor_to_c({output_arrayref_name}, c_outputs[{idx}]);"
            )
        ...
        for idx, output in enumerate(output_refs):
            ...
            with code.indent():
                ...
                if is_constant_buffer:
                    ...
                    code.writeline(f"{cached_output_name} = {cached_output_name}_tmp;")
                else:
            ...
            with code.indent():
                cached_output_name = f"cached_output_{next(self.cached_output_id)}"
                code.writeline(
                    f"thread_local ThreadLocalCachedOutputArray<std::decay_t<decltype({output})>> "
                ...
                output_arrayref_name = f"output_arrayref_{idx}"
                code.writeline(
                    f"using {output_arrayref_type} = std::tuple_element_t<{idx}, AOTInductorModelOutputs>;"
                )
                code.writeline(
                    f"using {output_element_type} = typename {output_arrayref_type}::value_type;"
                )
                code.writeline(
                    f"auto {output_arrayref_name} = {cached_output_name}.arrayref_tensor<{output_element_type}>();"
                )
                code.writeline(
                    f"torch::aot_inductor::arrayref_tensor_to_c({output_arrayref_name}, c_outputs[{idx}]);"
                )
